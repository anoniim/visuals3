...
class Prsoid : BaseSketch(
    ...
    private val radiusBoob = 200f
    private val radiusTit = radiusBoob / 10f

//        private val prsoid = mutableListOf<PVector>()
...
//        smooth(4)
    ...
    override fun draw() {
        translate(width / 2f, 2 * height / 3f)
        background(grey3)

//        constructPrsoidShape()
//        noLoop()
        drawPrsoidShape()
    }
    ...
    private fun drawPrsoidShape() {
        noFill()
        strokeWeight(3f)
        stroke(grey11)

//        beginShape()
//        for (vector in prsoid) {
//            vertex(vector.x, vector.y)
//        }
//        endShape()

        val vertex = prsoid[index]
        point(vertex.x, vertex.y)
        index++
    }

    private fun constructPrsoidShape() {
        noFill()
        // First half
        stroke(grey11)
        beginShape()
        var a = PI
        while (a < PI + HALF_PI) {
            val x = radiusBoob * cos(a)
            val y = radiusBoob * sin(a)
            vertex(x, y)
            prsoid.add(PVector(x, y))
            a += 0.01f
        }
        endShape()
        ...
        beginShape()
        a = HALF_PI
        while (a > HALF_PI - TWO_PI) {
            val x = radiusTit * cos(a)
            val y = -(radiusBoob + radiusTit) + radiusTit * sin(a)
            vertex(x, y)
            prsoid.add(PVector(x, y))
            a -= 0.1f
        }
    ...
    private val prsoid = mutableListOf(
